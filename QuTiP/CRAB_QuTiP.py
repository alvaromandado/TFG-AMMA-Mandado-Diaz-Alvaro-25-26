# CÓDIGO PARA EL CÁLCULO NUMÉRICO CON CRAB Y EL PAQUETE QUTIP
...
# Definimos las puertas lógicas a utilizar mediante QuTiP
I, sx, sy, sz = qt.qeye(2), qt.sigmax(), qt.sigmay(), qt.sigmaz()

# Hamiltoniano de deriva original
H_d = J_const * qt.tensor(sz, sz)
...
def ejecutar_intento_crab(intento_id, semilla):
    ...
    def evaluar_funcional(coeficientes_flat):
        ...
        # Evolución temporal usando objetos cuánticos de QuTiP
        U = qt.tensor(I, I)
        for j in range(N):
            H_total = H_d + sum(u[k, j] * H_c[k] for k in range(n_controles))
            U = (-1j * H_total * dt).expm() * U
            
        # Fidelidad Original (Parte real de la traza)
        g = -np.real((U_target.dag() * U).tr())
    ...
    # Matriz final
    U_final_optimo = qt.tensor(I, I)
    for j in range(N):
        H_total = H_d + sum(u_opt[k, j] * H_c[k] for k in range(n_controles))
        U_final_optimo = (-1j * H_total * dt).expm() * U_final_optimo
...
if __name__ == '__main__':
    ...
    print("\n" + "="*65)
    print("Resultados CRAB (Nelder-Mead con QuTiP)")
    print("-" * 65)
    ...
    print(np.round(mejor_resultado['U_final'].full(), decimals=3))
    ...
    ax[0].plot(tiempos, u_opt[0, :], label=r'$u_0(t)$, $\mathbb{I}\otimes\mathbb{I}$', color='#7f7f7f')
    ax[0].set_ylabel('Amplitud')
    ax[0].legend(loc='upper right')
    ax[0].grid(True, alpha=0.3)
    ax[0].set_title(f'Controles óptimos numéricos (CRAB con QuTiP, intento: {mejor_resultado["Intento"]}) ($J$ = {mejor_J:.3f})')

    # Subplot 1: u1 y u2
    max_u12 = np.max(np.abs(u_opt[1:3, :])) if np.max(np.abs(u_opt[1:3, :])) > 0 else 1.0
    ...
    ax[1].plot(tiempos, u_opt[1, :], label=r'$u_1(t)$, $\sigma_x\otimes1$', color='#1f77b4')
    ax[1].plot(tiempos, u_opt[2, :], label=r'$u_2(t)$, $\sigma_y\otimes1$', color='#ff7f0e')
    ax[1].set_ylabel('Amplitud')
    ax[1].legend(loc='upper right')
    ax[1].grid(True, alpha=0.3)

    # Subplot 2: u3 y u4
    max_u34 = np.max(np.abs(u_opt[3:5, :])) if np.max(np.abs(u_opt[3:5, :])) > 0 else 1.0
    ...
    ax[2].plot(tiempos, u_opt[3, :], label=r'$u_3(t)$, $1\otimes\sigma_x$', color='#2ca02c')
    ax[2].plot(tiempos, u_opt[4, :], label=r'$u_4(t)$, $1\otimes\sigma_y$', color='#d62728')
